...
class bounded_absence_after_q:
	...
	# 	return "q -> (always[{a},{b}](not p))".format(b=upper_bound)

	@staticmethod
	def generate_formula(upper_bound):
		return "(once[<={b}](q)) -> ((not p) since q)".format(b=upper_bound)
...
class bounded_absence_before_r:
	...
	@staticmethod
	def generate_future_formula(upper_bound):
		return "eventually[<={b}](r) -> (not p until r)".format(a=lower, b=upper_bound)

	@staticmethod
	def generate_formula(upper_bound):
		return "r -> (always[<={b}](not p))".format(b=upper_bound)
...
class bounded_absence_between_q_and_r:
	...
	def generate_future_formula(lower_bound, upper_bound):
		return "(r && !q && eventually q ) -> (not p until[{a},{b}] q)".format(a=lower_bound, b=upper_bound)

	@staticmethod
	def generate_formula(lower_bound, upper_bound):
		return "(r && !q && once q ) -> ((not p) since[{a},{b}] q)".format(a=lower_bound, b=upper_bound)
