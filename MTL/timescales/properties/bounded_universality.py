...
class bounded_universality_after_q:
	...
	# 	return "q -> (always[{a},{b}] p)".format(b=upper_bound)

	@staticmethod
	def generate_formula(upper_bound):
		return "(once[<={bound}](q)) -> (p since q)".format(bound=upper_bound)
...
class bounded_universality_before_r:
	...
	@staticmethod
	def generate_future_formula(upper_bound):
		return "eventually[<={b}](r) -> (p until r)".format(a=lower, b=upper_bound)

	@staticmethod
	def generate_formula(upper_bound):
		return "r -> (always[<={b}](p))".format(b=upper_bound)
...
class bounded_universality_between_q_and_r:
	...
	def generate_future_formula(lower_bound, upper_bound):
		return "(r && !q && eventually q) -> (p until[{a},{b}] q)".format(a=lower_bound, b=upper_bound)

	@staticmethod
	def generate_formula(lower_bound, upper_bound):
		return "(r && !q && once q) -> (p since[{a},{b}] q)".format(a=lower_bound, b=upper_bound)
